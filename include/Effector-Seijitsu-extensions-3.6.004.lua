...
-- Dado que "text", "shape" y "color" ya se inicializaron en utils-lib,
-- simplemente les agregamos índices.
---------------------------------------------------------------------
...
-- Función Hash Determinística: Aleatoriedad estática según posición
...
-- Generador de círculos perfectos en formato ASSDraw
...
shape.mesh_engines["honeycomb"] = shape.mesh_engines["hex"]
...
shape.mesh_engines["triangle"] = shape.mesh_engines["tri"]
...
shape.mesh_engines["diamond"] = shape.mesh_engines["dia"]
...
shape.mesh_engines["rings"] = shape.mesh_engines["ring"]
shape.mesh_engines["circle"] = shape.mesh_engines["ring"]
...
shape.mesh_engines["radial"] = shape.mesh_engines["ray"]
...
shape.mesh_engines["quadtree"] = shape.mesh_engines["quad"]
...
shape.mesh_engines["wave"] = shape.mesh_engines["bez"]
...
shape.mesh_engines["voronoi"] = shape.mesh_engines["vor"]
...
shape.mesh_engines["ashlar"] = function(box, cfg, px, py)
    ...
    for r = r_start, r_end do
        ...
        for c = c_start, c_end do
            ...
            -- Si el ladrillo cae dentro de la caja visible, lo dibujamos
...
shape.mesh_engines["delaunay"] = function(box, cfg, px, py)
    ...
    -- Función para obtener el punto pseudo-aleatorio único de cada celda
    local function get_pt(c, r)
        local rx = shape.mesh_utils.hash_rand(c, r, 2024)
        local ry = shape.mesh_utils.hash_rand(r, c, 4202)
        -- Lo devolvemos ya convertido al espacio local de la shape
        return c * cell_size + rx * cell_size - px, r * cell_size + ry * cell_size - py
    end
    
    -- Función auxiliar para distancia al cuadrado (es más rápido que usar raíz cuadrada)
    local function dist_sq(x1, y1, x2, y2)
        return (x2 - x1)^2 + (y2 - y1)^2
    end

    -- Recorremos la cuadrícula y armamos los triángulos
    for c = c_start, c_end do
        for r = r_start, r_end do
            -- Obtenemos los 4 puntos semilla que forman un "cuadrilátero" orgánico
            ...
            -- Elegimos la diagonal más corta para maximizar los ángulos (Principio Delaunay)
            ...
            if d1 < d2 then
                ...
                -- Triángulo 1: Top-Left, Top-Right, Bottom-Left
                tri1 = string.format("m %.3f %.3f l %.3f %.3f l %.3f %.3f l %.3f %.3f", 
                    tl_x, tl_y, tr_x, tr_y, bl_x, bl_y, tl_x, tl_y)
                -- Su Centroide
                cx1, cy1 = (tl_x + tr_x + bl_x)/3, (tl_y + tr_y + bl_y)/3
                
                -- Triángulo 2: Top-Right, Bottom-Right, Bottom-Left
                tri2 = string.format("m %.3f %.3f l %.3f %.3f l %.3f %.3f l %.3f %.3f", 
...
shape.mesh_engines["scallop"] = function(box, cfg, px, py)
    ...
    -- Si hay ángulo, expandimos los límites del cálculo rotando inversamente las esquinas de la caja
    if Angle == 0 then
    ...
    for r = r_start, r_end do
        for c = c_start, c_end do
            ...
            -- Dibujamos la escama base orientada hacia arriba
            ...
            -- Aplicamos la rotación global pivotando en 0,0 (lo que localmente es -px, -py)
            if Angle ~= 0 then
                scallop_code = shape.rotate(scallop_code, Angle, -px, -py)
                -- Rotamos también el centroide para que la separación direccional siga siendo precisa
            ...
            -- Descartamos celdas que por la rotación hayan caído extremadamente lejos de la caja visible
            -- Usamos un margen seguro de 2 Radios para no recortar bordes por error
...
shape.mesh_engines["web"] = function(box, cfg, px, py)
    ...
    -- El epicentro del impacto. Si no se especifica, es el centro de la letra/sílaba
    local cx = tonumber(cfg[4]) or box.cx
    local cy = tonumber(cfg[5]) or box.cy
    
    -- Calculamos el radio máximo necesario para cubrir toda la caja desde el epicentro
    local max_dist = 0
    local corners = {
    ...
    -- Función que calcula los vértices fracturados
    local function get_vertex(ring, ray)
        if ring == 0 then return cx, cy end -- El epicentro es fijo
        
        local safe_ray = ray % num_rays -- Conecta el último rayo con el primero (Cierra el círculo 360°)
        
        local a = safe_ray * step_A
        local r = ring * step_R
        
        -- Distorsión (Caos) determinística para que la forma se mantenga estable
    ...
    -- Trazamos la red celda por celda
...
shape.mesh_engines["rhombille"] = function(box, cfg, px, py)
    ...
    -- Cálculos para hexágonos puntiagudos (Pointy-topped)
    ...
    for r = r_start, r_end do
        ...
        for c = c_start, c_end do
            ...
            -- Las filas impares se desplazan medio ancho a la derecha
            ...
            -- CARA 2: Lado Derecho (Rombo Derecho)
            ...
            -- CARA 3: Lado Izquierdo (Rombo Izquierdo)
...
shape.mesh_engines["octagon"] = function(box, cfg, px, py)
    ...
    for r = r_start, r_end do
        for c = c_start, c_end do
            ...
            -- PIEZA 2: CUADRADO (El pequeño diamante que une 4 octágonos)
            -- Su centro está desplazado R píxeles en X e Y respecto al octágono
            local sq_cx = cx + R
            local sq_cy = cy + R
...
shape.mesh_engines["kagome"] = function(box, cfg, px, py)
    ...
    for r = r_start, r_end do
        -- Compensación del "Shear" (corte sesgado) para mantener el teselado recto
        ...
        for c = c_start, c_end do
            ...
            -- PIEZA 2: TRIÁNGULO HACIA ABAJO (Esquina inferior derecha)
            ...
            -- PIEZA 3: TRIÁNGULO HACIA ARRIBA (Esquina superior derecha)
...
shape.mesh_engines["jigsaw"] = function(box, cfg, px, py)
    ...
    -- Función paramétrica para trazar un borde clásico de rompecabezas
    local function get_jigsaw_edge(etype, c, r, x1, y1, x2, y2)
        -- Hash determinístico para decidir si la "cabeza" sale o entra, y su posición exacta
        local rand1 = shape.mesh_utils.hash_rand(c, r, etype == "H" and 1 or 2)
        local rand2 = shape.mesh_utils.hash_rand(c, r, etype == "H" and 3 or 4)
        
        local dir = (rand1 > 0.5) and 1 or -1
        local offset = (rand2 - 0.5) * 0.2 -- Desvía la cabeza entre un -10% y un +10% del centro
        ...
        -- Proporciones matemáticas de la cabeza (Bulbo)
        ...
        -- Cálculo de los vértices Bézier para crear el "cuello" y la "cabeza" redonda
        ...
        -- Trazado en sentido horario (Forward)
        ...
        -- Trazado en sentido antihorario (Backward) para que la pieza adyacente encaje perfecto
...
shape.mesh_engines["truchet"] = function(box, cfg, px, py)
    ...
    -- Constante mágica para trazar cuartos de círculo perfectos en Bézier
    ...
    for c = c_start, c_end do
        for r = r_start, r_end do
            ...
            -- Moneda aleatoria (Hash determinístico)
            local config = shape.mesh_utils.hash_rand(c, r, 789) > 0.5 and 1 or 0
            
            if config == 0 then
                -- CONFIGURACIÓN 0: Arcos en las esquinas Top-Left y Bottom-Right
                -- Máscara 1: Esquina Top-Left
                ...
                -- Máscara 2: Esquina Bottom-Right
                ...
                -- Máscara 3: Banda central ondulada
                ...
                -- CONFIGURACIÓN 1: Arcos en las esquinas Top-Right y Bottom-Left
                -- Máscara 1: Esquina Top-Right
...
shape.mesh_engines["circuit"] = function(box, cfg, px, py)
    ...
    local W = math.max(10, tonumber(cfg[1]) or 20)
    -- El grosor ahora es en PÍXELES absolutos (Recomiendo 2 a 6)
    ...
    for c = c_start, c_end do
        for r = r_start, r_end do
            ...
            -- CUADRADO BASE (El bloque de la celda sólido)
            ...
            -- Añadimos la bolita de soldadura a la pista si toca
            ...
            if trace_code ~= "" then
                -- MAGIA NEGATIVA: Al cuadrado sólido le restamos la pista.
...
shape.mesh_engines["pattern"] = function(box, cfg, px, py)
    ...
    -- Preparamos el "Sello" escalándolo al tamaño deseado y centrándolo
    -- shape.size y shape.displace("incenter") son herramientas nativas del KE
    ...
    for r = r_start, r_end do
        -- Desfase opcional para que los patrones (como flores) no queden en cuadrícula aburrida
        ...
        for c = c_start, c_end do
            ...
            -- Colocamos el sello en el centro de esta celda
...
function shape.slice_mesh( Shape_or_Text, Mode, Size, Overlap, Separation, Scale )
    ...
    -- FIX: Convertimos todos los valores a string de forma segura antes de concatenar
    local cfg_str = ""
